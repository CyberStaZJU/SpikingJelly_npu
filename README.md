...
Qualified native inputs are rank-two-or-higher FP32 time-major sequences with a non-empty time dimension and non-empty flattened timestep, contiguous storage-offset-zero NPU storage, and a spiking `surrogate.ATan`. BF16 public IF/LIF/KLIF/PLIF and FedSNN decay-LIF tensors are supported by an explicit `bf16-public-fp32-aspy-island`: inputs are converted and revalidated before launch, native recurrence remains FP32, public spikes return as BF16, and runtime voltage plus PLIF/KLIF master parameters, gradients, and optimizer state remain FP32. Configure an AsPy module before dtype conversion and create its optimizer only after backend/dtype setup; a late Torch-to-AsPy switch with a non-FP32 master scalar is rejected before extension loading rather than silently desynchronizing existing optimizer state. This is mixed-precision boundary support, not BF16-native AsPy kernel I/O or arithmetic. LIF and KLIF require a fixed Python float `tau > 1`; KLIF keeps public learnable scalar `k` live and expands it into an aligned device block only at the native boundary, while PLIF transports reciprocal tau as a one-element contiguous FP32 NPU tensor so `w` and `w.grad` remain live. The native bridge itself accepts only physical `ACL_FORMAT_ND` storage. The FedSNN adapter additionally recognizes real convolutional rank-5 `ACL_FORMAT_NCDHW` outputs, reshapes and clones them into fresh ND storage before launch, and rejects other physical formats before loading or calling the extension. The FedSNN-specific stateless path preserves the application's exact charge ordering, detached soft reset, ATan derivative, and zero membrane at each public forward; it is exposed as `spikingjelly_npu.fedsnn.DecayLIF`. Only first-order gradients are supported. FP16, non-ATan or non-spiking surrogates, unsupported layouts, and arbitrary dynamic-shape graphs use observable pre-execution fallback or strict failure. Stateful IF/LIF/PLIF nodes in single-step mode intentionally remain on PyTorch even when `backend_strict=True`; strict native enforcement applies to qualified multi-step requests such as `packed_aspy`.
...
# Explicit mixed precision is fail-closed at runtime capability setup.
...
with npu_bf16_autocast():
    logits = model(torch.randn(8, 3, 224, 224, device=device))
```

This release stage establishes eager FP32 semantics, public namespaces, state-dict behavior, CPU tests, observable routing, and import safety. For standard RNN/GRU/LSTM, actual FP32 NPU inputs use a functional decomposition because the examined CANN 8.5 fused `DynamicGRUV2` route rejects FP32. The decomposition precomputes per-layer/direction input affine work and keeps hidden recurrence in eager primitives; it is a compatibility/availability path, not an acceleration claim, and it does not use `DynamicGRUV2`. Outside the explicit package-managed BF16 profile, FP16/BF16 and every non-NPU route delegate upstream unchanged. The opt-in `npu_bf16_autocast()` profile fixes NPU autocast to BF16 with caching disabled. Conv/Linear/MatMul and public activations use BF16 where physically qualified; standard FP32-storage recurrent modules retain the eager compatibility decomposition so affine operators may autocast to BF16 while gates/state stay FP32. LayerNorm is targeted for conservative FP32 execution, while standard Transformer Softmax remains delegated to the qualified torch-npu attention implementation until an explicit FP32-softmax route is separately proven. Project spiking recurrent state/gates, neuron membrane/threshold/surrogate/reset, BatchNorm/BNTT, average reductions, and final logits remain FP32. Master parameters, gradients, and optimizer state remain FP32. FP8 is not enabled by this profile and requires a separate capability and numerical qualification. This source-level contract remains model- and shape-specific rather than a universal BF16 claim. Graph capture, arbitrary dynamic-shape, compiled execution, and higher-order-gradient claims are excluded. Spiking recurrent equations are project-defined extensions. Spikformer factories provide architecture/configuration compatibility; strict external checkpoint-key/layout compatibility is not claimed. TorchScript is also outside the current recurrent semantic-alpha contract because support varies across PyTorch releases.

Physical BF16 eager validation on the qualified stack now covers 84 tiny-shape cases across standard RNN/GRU/LSTM, MultiheadAttention, Transformer encoder/decoder layers and stacks, top-level Transformer, project spiking recurrent modules, SpikingSelfAttention, and Spikformer. Train/eval and full/remainder/singleton batches passed with FP32 master parameters, gradients, and optimizer state. This is an eager availability and dtype-contract result, not representative-shape, convergence, NPUGraph, native Spikformer, latency, memory, FP8, or architecture-wide qualification. The earlier FP32 Conv-policy investigation is retired from the active API and support claim. See [`docs/sequence-acceleration-contract.md`](docs/sequence-acceleration-contract.md) and [`bf16_mixed_acceptance_policy_20260804.json`](docs/evidence/bf16_mixed_acceptance_policy_20260804.json).
...
Real fixed-shape NPUGraph is qualified for the previously documented AsPy IF, LIF, and PLIF paths. The general bucket machinery still requires a physical CANN/torch-npu canary for every newly claimed model path. Spikformer evaluation now has such shape-specific evidence at BF16 batches 8 and 64: changed-input replay was exact and strict mismatched batches were rejected before execution. Batch 8 replay passed its latency gate; batch 64 was about 5% slower and is correctness-only. Spikformer training capture failed after launch and poisoned the runner without eager replay, so it remains unsupported. Separate train/eval captures are required. Models must declare graph-safe per-forward state or callers must explicitly opt in after verifying it. Training capture is disabled by default; the qualified opt-in path requires deterministic algorithms unless the expert-only override is used. Higher-order differentiation and arbitrary dynamic shapes are unsupported.
...
- the standard recurrent decomposition is selected for actual FP32 NPU input storage, including under `npu_bf16_autocast()`; in that profile affine operators may autocast to BF16 while recurrent gates/state remain FP32, avoiding the unavailable FP32 `DynamicGRUV2` route. This remains an availability contract rather than a speedup, graph, or arbitrary dynamic-shape claim;
- standard Transformer wrappers also delegate BF16 attention/normalization/reduction policy to upstream torch-npu; source-level master-state tests do not replace physical NPU qualification;
...
- tiny-shape BF16 SpikingSelfAttention/Spikformer train/eval execution is qualified for the tested matrix; longer optimizer trajectories, convergence, native routing, graph capture, representative shapes, FP8, and acceleration remain unqualified;
