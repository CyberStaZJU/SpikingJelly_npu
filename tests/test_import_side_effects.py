...
def test_import_does_not_import_torch_npu():
    code = (
        ...
        "BF16CapabilityStatus, GraphBucketSpec, StaticGraphRunner, npu_bf16_autocast, "
        "get_npu_autocast_state, is_npu_bf16_autocast_active); "
        "assert BF16CapabilityStatus.SUPPORTED.value == 'supported'; "
        "assert callable(npu_bf16_autocast); "
        "assert not get_npu_autocast_state().enabled; "
