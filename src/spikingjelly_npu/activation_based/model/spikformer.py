...
class SpikformerPatchStem(nn.Module, base.MultiStepModule):
    ...
    def forward(self, x_seq: Tensor) -> Tensor:
        ...
        if x_seq.shape[2] != self.in_channels:
            raise ValueError(
                f"expected patch-stem C={self.in_channels}, but got C={x_seq.shape[2]} "
                f"in shape={tuple(x_seq.shape)}"
            )
    ...
    def _to_sequence(self, x: Tensor) -> Tensor:
        if x.ndim == 4:
            if x.shape[1] != self.in_channels:
                raise ValueError(
                    f"expected 4D input C={self.in_channels}, but got C={x.shape[1]} "
        ...
        if x.ndim == 5:
            if x.shape[2] != self.in_channels:
                raise ValueError(
                    f"expected 5D input C={self.in_channels}, but got C={x.shape[2]} "
                    f"in shape={tuple(x.shape)}"
                )
            return x
        raise ValueError(
