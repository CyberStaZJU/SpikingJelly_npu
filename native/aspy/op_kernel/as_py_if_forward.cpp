...
class KernelAsPyIfForward {
 public:
  __aicore__ inline KernelAsPyIfForward() = default;
  ...
  __aicore__ inline void Init(
      ...
      GM_ADDR v_final,
      GM_ADDR h_seq,
      const AsPyIfForwardTilingData& tiling) {
 ...
 private:
  ...
  __aicore__ inline void ProcessTile(uint32_t begin, uint32_t count) {
    // The Python adapter pads every row to eight FP32 values, and tile starts
    // and lengths use the same 32-byte granularity. All GM DMA ranges are
    // therefore disjoint and naturally aligned across AI Vector cores.
    ...
    for (uint32_t time = 0; time < time_steps_; ++time) {
      ...
      if (hard_reset_) {
        Duplicate(spike_, reset_, static_cast<int32_t>(count));
        PipeBarrier<PIPE_V>();
        Select(
            voltage_,
            mask_,
        ...
        Muls(spike_, spike_, threshold_, static_cast<int32_t>(count));
        PipeBarrier<PIPE_V>();
        Sub(voltage_, voltage_, spike_, static_cast<int32_t>(count));
      }
      PipeBarrier<PIPE_V>();
...
extern "C" __global__ __aicore__ void as_py_if_forward(
    GM_ADDR xSeq,
    GM_ADDR vInit,
    GM_ADDR spikeSeq,
    GM_ADDR vSeq,
