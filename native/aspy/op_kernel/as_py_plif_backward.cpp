...
class KernelAsPyPlifBackward {
 public:
  __aicore__ inline KernelAsPyPlifBackward() = default;
  ...
  __aicore__ inline void Init(
      ...
      GM_ADDR reciprocal_tau,
      GM_ADDR grad_x_seq,
      GM_ADDR grad_v_init,
      GM_ADDR grad_reciprocal_tau_partial,
      const AsPyPlifBackwardTilingData& tiling) {
    ...
    reciprocal_tau_gm_.SetGlobalBuffer(
        reinterpret_cast<__gm__ float*>(reciprocal_tau));
    ...
    grad_reciprocal_tau_partial_gm_.SetGlobalBuffer(
        reinterpret_cast<__gm__ float*>(grad_reciprocal_tau_partial));
    ...
    pipe_.InitBuffer(calc_buffer_, 4U * buffer_bytes);
  ...
  __aicore__ inline void Process() {
    ...
    DataCopy(scalar, reciprocal_tau_gm_[0], 8);
    scalar_queue_.EnQue(scalar);
    scalar = scalar_queue_.DeQue<float>();
    reciprocal_tau_ = scalar.GetValue(0);
    scalar_queue_.FreeTensor(scalar);
    decay_ = 1.0f - reciprocal_tau_;
    input_scale_ = decay_input_ ? reciprocal_tau_ : 1.0f;
 ...
 private:
  ...
  __aicore__ inline void ProcessTile(uint32_t begin, uint32_t count) {
    ...
    for (uint32_t reverse = 0; reverse < time_steps_; ++reverse) {
      ...
      Adds(v_prev, v_prev, -reset_, static_cast<int32_t>(count));
      PipeBarrier<PIPE_V>();
      if (decay_input_) {
        Sub(v_prev, input, v_prev, static_cast<int32_t>(count));
      } else {
        Muls(v_prev, v_prev, -1.0f, static_cast<int32_t>(count));
      }
      PipeBarrier<PIPE_V>();
      Mul(v_prev, v_prev, carry_, static_cast<int32_t>(count));
      PipeBarrier<PIPE_V>();
      Add(partial_, partial_, v_prev, static_cast<int32_t>(count));
      PipeBarrier<PIPE_V>();
    ...
    CopyOut(grad_reciprocal_tau_partial_gm_, begin, partial_, count);
  }
  ...
  GlobalTensor<float> reciprocal_tau_gm_;
  GlobalTensor<float> grad_x_seq_gm_;
  GlobalTensor<float> grad_v_init_gm_;
  GlobalTensor<float> grad_reciprocal_tau_partial_gm_;
...
extern "C" __global__ __aicore__ void as_py_plif_backward(
    GM_ADDR xSeq,
    GM_ADDR vPrevSeq,
    GM_ADDR hSeq,
    GM_ADDR spikeSeq,
    ...
    GM_ADDR gradVSeq,
    GM_ADDR gradVFinal,
    GM_ADDR reciprocalTau,
    GM_ADDR gradXSeq,
    GM_ADDR gradVInit,
    GM_ADDR gradReciprocalTauPartial,
    GM_ADDR workspace,
    GM_ADDR tiling) {
  KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_AIV_ONLY);
  GET_TILING_DATA(tiling_data, tiling);
  KernelAsPyPlifBackward kernel;
  kernel.Init(
      xSeq, vPrevSeq, hSeq, spikeSeq, gradSpikeSeq, gradVSeq, gradVFinal,
      reciprocalTau, gradXSeq, gradVInit, gradReciprocalTauPartial,
      tiling_data);
  kernel.Process();
}
