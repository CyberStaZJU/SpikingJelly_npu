...
class KernelAsPyLifBackward {
 public:
  __aicore__ inline KernelAsPyLifBackward() = default;
  ...
  __aicore__ inline void Init(
      ...
      GM_ADDR grad_x_seq,
      GM_ADDR grad_v_init,
      const AsPyLifBackwardTilingData& tiling) {
 ...
 private:
  ...
  __aicore__ inline void ProcessTile(uint32_t begin, uint32_t count) {
    ...
    for (uint32_t reverse = 0; reverse < time_steps_; ++reverse) {
      ...
      // carry is the post-state gradient. Form grad_h, then split it into the
      // input gradient and the previous-state carry with the exact LIF scales.
...
extern "C" __global__ __aicore__ void as_py_lif_backward(
    GM_ADDR hSeq,
    GM_ADDR spikeSeq,
