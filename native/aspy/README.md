...
The build script preflights Linux aarch64, CPython 3.10, torch/torch-npu 2.9, and a CANN 8.5 toolkit path. Set `PYTHON=/path/to/python3.10` to select the interpreter. It generates ACLNN projects with `msopgen`, installs the reviewed IF/LIF/KLIF/PLIF/FedSNN host, tiling, and AIV kernel sources, targets `ascend910b`, installs custom-op APIs under the external root, and builds `_spikingjelly_npu_aspy` with torch-npu `NpuExtension`. Before reporting success it imports the extension and requires all ten forward/backward symbols. External `build-manifest.json` records those verified capabilities, source Git identity and dirty state, and a deterministic source-input digest over each file's path, size, and SHA-256, plus runtime/tool versions, toolkit path, compiler, and target SOC. Under Git, the digest scope is every tracked plus untracked non-ignored file; a source snapshot uses the equivalent filtered source-file scope. No generated file is written into the source repository.
