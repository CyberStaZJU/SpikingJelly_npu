...
#include "as_py_plif_forward_tiling.h"
...
static ge::graphStatus TilingFunc(gert::TilingContext* context) {
  ...
  tiling.set_decayInput(*context->GetAttrs()->GetBool(3) ? 1U : 0U);
...
namespace ops {
class AsPyPlifForward : public OpDef {
 public:
  explicit AsPyPlifForward(const char* name) : OpDef(name) {
    ...
    this->Input("vInit").ParamType(REQUIRED).DataType({ge::DT_FLOAT})
        .Format({ge::FORMAT_ND}).UnknownShapeFormat({ge::FORMAT_ND});
    this->Input("reciprocalTau").ParamType(REQUIRED).DataType({ge::DT_FLOAT})
        .Format({ge::FORMAT_ND}).UnknownShapeFormat({ge::FORMAT_ND});
