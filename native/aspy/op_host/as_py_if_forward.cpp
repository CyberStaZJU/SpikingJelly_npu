...
#include "as_py_if_forward_tiling.h"
...
static ge::graphStatus TilingFunc(gert::TilingContext* context) {
  ...
  AsPyIfForwardTilingData tiling;
  ...
  // The Python adapter pads every flattened time row to a 32-byte boundary.
  // Rejecting unaligned direct bridge calls keeps scalar GM writes well-defined.
  if ((neuron_count & 7U) != 0U) {
    return ge::GRAPH_FAILED;
  }
  // Neuron tiles are independent across the entire time recurrence. Assign
  // disjoint 32-byte-aligned tiles to separate AI cores; the kernel strides
  // over any remaining tiles when the shape exposes more than 20 tiles.
  constexpr uint32_t kAscend910BAiCores = 20U;
  context->SetBlockDim(std::min(tile_count, kAscend910BAiCores));
  tiling.SaveToBuffer(
...
namespace ops {
class AsPyIfForward : public OpDef {
 public:
  explicit AsPyIfForward(const char* name) : OpDef(name) {
