...
#include "as_py_lif_backward_tiling.h"
...
static ge::graphStatus TilingFunc(gert::TilingContext* context) {
  ...
  const float tau = *context->GetAttrs()->GetFloat(5);
  if (!(tau > 1.0f)) {
    return ge::GRAPH_FAILED;
  }
  const uint32_t tile_count =
      static_cast<uint32_t>((neuron_count + kTileLength - 1) / kTileLength);
  AsPyLifBackwardTilingData tiling;
  ...
  tiling.set_decayInput(*context->GetAttrs()->GetBool(6) ? 1U : 0U);
...
namespace ops {
class AsPyLifBackward : public OpDef {
 public:
  explicit AsPyLifBackward(const char* name) : OpDef(name) {
