...
#include "as_py_if_backward_tiling.h"
...
static ge::graphStatus TilingFunc(gert::TilingContext* context) {
  ...
  AsPyIfBackwardTilingData tiling;
  ...
  // The recurrence is serial only along time. Neuron tiles are independent,
  // 32-byte aligned, and can be assigned to distinct AI Vector cores.
...
static ge::graphStatus InferShape(gert::InferShapeContext* context) {
  ...
  *context->GetOutputShape(1) = *context->GetInputShape(4);
  return ge::GRAPH_SUCCESS;
}
...
namespace ops {
class AsPyIfBackward : public OpDef {
 public:
  explicit AsPyIfBackward(const char* name) : OpDef(name) {
    ...
    this->Input("gradSpikeSeq").ParamType(REQUIRED).DataType({ge::DT_FLOAT})
        .Format({ge::FORMAT_ND}).UnknownShapeFormat({ge::FORMAT_ND});
    this->Input("gradVSeq").ParamType(REQUIRED).DataType({ge::DT_FLOAT})
        .Format({ge::FORMAT_ND}).UnknownShapeFormat({ge::FORMAT_ND});
