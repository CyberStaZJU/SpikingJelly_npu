...
The public Python APIs are available independently of physical NPU qualification. Small-shape FP32 eager adaptation has passed for standard recurrent, project-defined spiking recurrent, and standard Transformer families. Tiny-shape SpikingSelfAttention/Spikformer train/eval parity also passed, but only after explicitly setting the process-global Conv policy with `configure_npu(allow_conv_hf32=False)`; the default Conv HF32 policy failed the tested Spikformer training-gradient gates and remains unqualified. Compact IF/LIF passed correctness but failed the performance gate. Any native, graph, memory, latency, convergence, or representative-shape claim remains capability- and shape-specific and must pass `docs/evidence/sequence_acceptance_policy.json`; the continuous-state exception is limited by `docs/evidence/sequence_acceptance_addendum_20260803.json`. Sanitized result identities and numerical boundaries are recorded in `docs/evidence/sequence_physical_qualification_20260803.json`.
...
Under torch-npu's default Conv HF32 policy, a localized tiny Spikformer train case passed public output (`1.4901e-8` maximum absolute difference), loss (`1.6451e-5`), persistent buffers, initial node-state gradients, and continuous final states (`3.5059e-4` under the addendum), but failed the unchanged gradient gates: 32/24576 input-gradient elements mismatched with maximum absolute difference `8.2284e-4`, and 7/29 parameter gradients failed, worst `1.3299e-2` at the first patch-stem Conv2d weight.

Boundary hooks showed the first Conv2d output itself stayed inside the tight gate (`1.8347e-5`), while training BatchNorm and MaxPool amplified upstream device drift; LIF spike outputs were exact and the local LIF vector-Jacobian product passed. Explicit NPU BatchNorm arithmetic, a diagnostic CPU-authority BatchNorm autograd implementation, and strict deterministic algorithms did not repair the end-to-end gradients. The first `torch.npu.conv.allow_hf32=False` attempt is invalid as parity evidence because it replaced rather than extended `PYTHONPATH`, removing the CANN `tbe` module before model execution.

A subsequent isolated probe preserved the CANN Python paths and directly repaired the localized case with Conv HF32 disabled. The policy was then productized as the explicit, no-import-side-effect runtime option `configure_npu(allow_conv_hf32=False)`. Its default is `None`, so package import and ordinary model construction do not change process-global Conv precision; an explicit request fails if torch-npu does not expose the setting or does not retain the assigned value.

Using clean commit `7cbb7f607034e3b2fe5e1039816beecdc0dbb55e` (tree `e0d080276eb2023063df45d34c89b125fc9b1850`) and a clean CANN 8.5 build with all 14 manifest symbols, the productized tiny-shape matrix passed 12/12 cases:

- SpikingSelfAttention: `T=3`, `C=8`, `L=5`, two heads, batches `4/3/1`;
- Spikformer: `T=2`, `3x32x32` input, `embed_dims=32`, four heads, one block, batches `4/3/1`;
- train and eval for each batch kind, with two SGD updates per training case.

Worst observed maximum absolute differences were `2.3693e-6` for parameter gradients, `8.9407e-7` for continuous node final state, `3.3528e-8` for input gradients, `1.3039e-8` for initial node-state gradients, `2.9802e-8` for public output, and `3.7253e-9` for optimizer/state-dict updates. The separate strict AsPy feasibility sentinels passed by rejecting unsupported default Sigmoid-surrogate and/or physical-format requests before extension loading or native launch; this does not qualify native Spikformer execution.

Therefore tiny-shape eager FP32 train/eval parity is supported **only with explicit Conv HF32-off**. The default Conv HF32 training path, longer optimizer trajectories, convergence, native Spikformer execution, graph capture, representative shapes, latency, memory, and family-wide acceleration remain unqualified. No additional tolerance relaxation is implied.
